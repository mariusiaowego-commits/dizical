...
async def ws_generate(websocket: WebSocket):
    """WebSocket 端点：实时转发 hermes chat 输出"""
    ...
    try:
        ...
        # 构建 prompt
        await websocket.send_json({"type": "status", "message": "构建 prompt..."})
        from src.report_templates import build_monthly_report_prompt
        prompt, aspect_ratio, report_data = build_monthly_report_prompt(year, month, style)
        await websocket.send_json({"type": "status", "message": f"Prompt 已构建（{len(prompt)} 字符）"})
        ...
        await websocket.send_json({"type": "status", "message": "hermes chat 已启动，实时输出："})

        # 实时读取 stdout
        async for line in process.stdout:
            text = line.decode("utf-8", errors="replace").rstrip()
            await websocket.send_json({"type": "output", "message": text})

        # 等待进程结束
        await process.wait()
        await websocket.send_json({"type": "status", "message": f"hermes 进程结束 (exit={process.returncode})"})

        # 读取 stderr（如果有错误）
        stderr = await process.stderr.read()
        if stderr:
            await websocket.send_json({"type": "error", "message": stderr.decode("utf-8", errors="replace")})

        # 从输出中提取图片路径
        # 需要重新读取 stdout（已经消费完了）
        # 这里简化处理：让用户看输出后手动刷新
        await websocket.send_json({"type": "done", "message": "生成完成，请刷新页面查看历史记录"})
