...
> 开发中 (未冻结): **v1.2.0-dev** (dad 2026-09-13 l1) —— 镭射/防伪箔层从画面窗搬上卡面级**铺满整卡** /
> 画面窗退化为纯定位容器(撤裁剪+四边压暗, 少掉一层圆角矩形) / 金环 **11px→6px** 且 5 环→3 环 /
> 页脚文字区加**主题化底衬渐隐** `--theme-foot-scrim`(深色暗渐隐 · 淡色亮渐隐), 说明栏底衬同步加强。
> 对照旧观感请开 v1.0.0 冻结页 (旧: 11px 金环 + 画面窗自成一层框)。

## 版本号规则 (改卡片 / 3D 样式前必读)

1. 动手前先看 demo 页右上角 chip，确认现在是第几版 (当前 `v1.2.0-dev`)。
