...
> 开发中 (未冻结): **v1.3.0-dev** (dad 2026-09-13 二轮) —— ①聚光灯总强度 `--ccg-light-gain` = **0.6**
> (跟随光标的柔光 + 主光两层同乘, 减弱 40%, 图案不再被冲淡) ②页脚行 (星级 + DIZICAL) **并入说明栏**.ccg-plate 内,
> 页脚不再自带底衬 (`--theme-foot-scrim` 已删), 说明栏下沿从 88% 伸到 `--foot-b` ⇒ 文字区底衬只剩一层。
> 对照旧观感请开 v1.0.0 冻结页 (旧: 11px 金环 + 画面窗自成一层框)。

## 版本号规则 (改卡片 / 3D 样式前必读)

1. 动手前先看 demo 页右上角 chip，确认现在是第几版 (当前 `v1.3.0-dev`)。
