## [2026-07-27] PR #178 今日总时长显眼卡片 + 7-27 data fix (已 merge main @ 13df74d)

**branch**: `fix/practice-today-summary-and-grouping` (基于 main @ b43bc3b, **squash merged PR #178 → 13df74d**)

**dad 反馈 3 轮** (21:30-22:00):
1. "你在练习里截图的, 今天 27 日不是联系了 60 分钟吗" → dad 误信截图 60, 实是 UI 渲染 bug
2. "萨丽哈明显是 2 个 11 分钟, 而且两次练习的细节是不同的" → 误以为两次各 11min
3. 最终: "萨丽哈是 5+6 一共 11 分钟, 一共 49 分钟" → 确认 49 是真, 60 是截图渲染重复算萨丽哈
4. "练习 tab 要给一个今天练习总时长的展示, 明显一点位置" → 加显眼卡片

**根因** (摸排 30 分钟):
- 截图 "60 分钟" = 吸气长音 10 + 单吐tuku 6 + 萨丽哈 11 + 西藏 14 + **萨丽哈 11** + 颤音 4 + 单吐练习 4 (萨丽哈被加两次)
- 真实 session SUM = 49 (吸气长音 10 + 单吐tuku 6 + 萨丽哈 11 + 西藏 14 + 颤音 4 + 单吐练习 4)
- 渲染代码 line 1722 (PR #176 前): group header "X 分钟" 从 daily.items 取该 item.minutes, 同 item_name 多个 session 重复算
- main 上 PR #176 (6c8ec0d) 已用 subjectTotals dict 修复 (group header 都显示 11, 加总 22 还是重复, 但 group header 一致)

**额外发现**:
- daily_practices 7-27 = 81 min, audit log 算出来: 1+3+5+3+5+3+5+3+5+3+5+3+5+5+5+5+5 = 81 (含 extra 残量)
- extra 路径写 daily.items 时不带 session_id, V3 session 路径累加 = 残量永远清不掉
- dad 拍 "明显 60 是对的, 81 是错的" → 但 60 是 UI 渲染 bug, 真值是 49
- dad 最后更正: "49 是对的, 萨丽哈 5+6=11"
...
2. **PR #178** (.today-summary-bar 显眼卡片):
...
3. **服务重启** (PID 5034 → 66394): 加载 PR #178 新代码, API 验证 total=49 = sessions sum

**关键沉淀** (skill 候选):
- "group header 渲染读 daily.items 残量" 是 V3 session 路径的隐性风险, 即使 PR #176 修了 subjectTotals dict, daily.items 仍是脏数据来源
...
- "你来判断" 配合 clarify 4 选项时, dad 拍板后必须复核数据, 防止 dad 误读截图
- Worktree rebase 冲突时 cherry-pick 比 rebase --abort + 重做 更可控
...
## [2026-07-27] phase2 web/mac 切云 — 备份 + PRD + 收尾 (实施暂缓)

**branch**: `feat/p4-phase2-web-mac-to-cloud` (基于 main @ b43bc3b, **未 commit 未 push**)

**dad 拍板 11 个决策** (Q1-Q11, 14:50-14:55 拍板):
- Q1=A cherry-pick 2 fix / Q2=手动激活 (不写 cron) / Q3=A 本地→云 INSERT IGNORE / Q4=A PRD 双写
- Q5=A 停用本地 (等 4 前提) / Q6=A mac app 同 PR / Q7=A 1 PR 完事 / Q8=C minip 暂缓
- Q9=A agent 一气呵成 mcp DESCRIBE / Q10=A 瞬时切云 / Q11=mac app 是 WKWebView 壳, 不动代码

**完成 3 件事** (本会话):
1. 本地 SQLite 备份 `data/backups/phase2-pre-cloud-20260727-154911/` 684K, PRAGMA integrity_check=ok
   - 13 表共 1560 行 (重: practice_sessions 668 / practice_audit_log 355 / daily_practices 267)
   - dad 7-17 红线 "本地永不删, 永久备份" ✅
2. phase2 PRD 双写 (Q4=A): `PRDs/AI-PRD-前后端统一云-phase2实施-260727.md` 249 行
   - Obsidian dizical + Obsidian minip 两个子目录, md5 三方一致 `e2e59896db3480960353a96e84447061`
   - 包含 Q1-Q11 全决策 + 6 步 PR-1 清单 + 6 类风险 + 5 分钟回滚 SOP
3. handoff 写好: `handoff-2026-07-27-p4-phase2.md` (~12K) — 完整下次接手指引

**关键发现 5 个** (实施前必看):
1. main 上 dual-backend 已就位 (database_mysql.py + db_adapter.py + factory 末尾) — skill 警告过时
2. staging 上 2 个 mysql fix (c46557c kwargs / 0af13d6 behavior_log) **没 merge main** — Q1=A 必 cherry-pick
3. MySQL 后端**无 CREATE TABLE 逻辑** — 7-27 新加的 practice_sessions + 冗余列不在云端, PR-1 第 2 步必须 mcp DESCRIBE + 写迁移脚本
4. minip 端 0 命中 practice_sessions, submitRecord 缺 `tempo_note/bpm/content` — Q5 前提 4 红线, 阶段 C 才进
5. mac app 在主仓 channels/mac-app, 是 WKWebView 壳 (DIZICAL_URL="http://127.0.0.1:8765") — **Q11 关键发现, 切云 = mac app 0 代码改动**

**PR-1 6 步** (除备份外全部暂缓):
1. ✅ 本地备份 (已完成)
2. ⏸ 写 `scripts/migrate_cloud_schema.py` (补 schema)
3. ⏸ 写 `scripts/sync_local_to_cloud_phase2.py` (INSERT IGNORE 全量)
4. ⏸ cherry-pick 2 fix (c46557c/0af13d6)
5. ⏸ mac kid_app 加 DATABASE_URL env + 切云 + web 录入验证 + mac app 拉起
6. ⏸ 写 `scripts/cloud_backup.sh` (Q2: dad 手动激活)

**dad 14:55**: "这个分支先暂缓, 写好 handoff. 后续再接续开发" — agent 立即停手, 写 handoff 收尾

**下次接手指引**: 读 handoff §"下次接手指引" (3 步: 确认状态 → 重读 PRD → 摸清现状) + 按 PR-1 6 步顺序实施

**未 commit 未 push** — 留给下次会话接续 (PRD + handoff + 备份目录 3 件套都在 working tree)
