...
**2026-05-26 — 缴费提醒逻辑修正**:
- **Bug**: 原逻辑"最后上课日前1天"，4月最后周六=4/25=最后上课日恰好对的上，但5月最后周六=5/30（无课），误导用户
- **修复**: 改为"当月最后一个周六前3天"提醒，CLI和PaymentManager两处同步修改
- **验证**: pytest 49/49 ✅，manual test 5月最后周六=5/30，提醒日=5/27 ✅
- **教训**: Python weekday() 5=Saturday 不是 4（4=Friday）
- **commit**: `d323cfc` → **已合并 main**

**2026-05-25 — 提前结束计时 bug 修复 + 弹窗确认**:
- **Bug 修复**:
  1. `finishEarly()` 无防双击守卫，iPad 双击会覆盖正确时间为 1 分钟
  2. `submitPractice()` 无 try/catch，网络异常后 `submitting` 卡死，打卡功能永久瘫痪
  3. 打卡成功后 `innerHTML` 覆盖删除了 `confirmMins` 元素，第二次打卡 `getElementById` 返回 null 报错
  4. 暂停态 `started=false, timerRunning=false` 被误拦截
  5. 打卡后 2500ms `setTimeout` 无条件 `resetTimer`，把用户已开始的新计时清零
- **功能**: 提前结束改为弹窗二次确认，显示练习分钟数 + gsap 入场动效
- **交互**: 确认打卡 → 提交 + 庆祝动效；继续练习 → 恢复计时
- **commit**: `3bb67f0` + `741507d` + `e988e7c` + `bb6b0a5`
- **PR**: #37 (已合并)
