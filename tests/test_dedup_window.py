...
def test_dedup_check_returns_none_first_time():
    ...
    assert _check_dedup("2026-07-29", 909, 5) is None


def test_record_dedup_then_check_within_5s_returns_cached():
    """5s 内: 返缓存."""
    body = {"ok": True, "session": {"id": 100, "duration_minutes": 5}}
    _record_dedup("2026-07-29", 909, 5, body)
    cached = _check_dedup("2026-07-29", 909, 5)
...
def test_dedup_different_minutes_does_not_match():
    """不同 minutes 不命中缓存."""
    _record_dedup("2026-07-29", 909, 5, {"ok": True, "session": {"id": 100}})
    assert _check_dedup("2026-07-29", 909, 10) is None


def test_dedup_different_item_id_does_not_match():
    """不同 item_id 不命中缓存."""
    _record_dedup("2026-07-29", 909, 5, {"ok": True, "session": {"id": 100}})
    assert _check_dedup("2026-07-29", 910, 5) is None


def test_dedup_different_date_does_not_match():
    """不同 date 不命中缓存 (PR-A 架构修复)."""
    _record_dedup("2026-07-29", 909, 5, {"ok": True, "session": {"id": 100}})
    assert _check_dedup("2026-07-30", 909, 5) is None


def test_dedup_check_does_not_record():
    """_check_dedup 只读, 不写缓存."""
    initial = _check_dedup("2026-07-29", 911, 7)
    assert initial is None
    again = _check_dedup("2026-07-29", 911, 7)
    assert again is None


def test_dedup_check_zero_values():
    """item_id 或 minutes 为 0 → 返 None (不参与 dedup)."""
    _record_dedup("2026-07-29", 912, 5, {"ok": True})
    assert _check_dedup("2026-07-29", 0, 5) is None
    assert _check_dedup("2026-07-29", 912, 0) is None


def test_dedup_empty_date_returns_none():
    """date 为空 → 返 None."""
...
def test_dedup_expires_after_window():
    ...
    try:
        _record_dedup("2026-07-29", 913, 5, {"ok": True})
        assert _check_dedup("2026-07-29", 913, 5) is not None
        time.sleep(0.2)
        assert _check_dedup("2026-07-29", 913, 5) is None
    finally:
